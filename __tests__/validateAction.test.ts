...
describe('validateAction', (): void => {
  ...
  let mockStdOut: jest.SpyInstance<boolean, [str: string | Uint8Array, encoding?: BufferEncoding, cb?: (err?: Error) => void]>
  ...
  describe('no github token', () => {
    ...
    test('writes to stdout with an error message', () => {
      expect(mockStdOut).toHaveBeenCalledTimes(1)
      expect(mockStdOut).toHaveBeenCalledWith('::error::GITHUB_TOKEN environment variable not set\n')
    })
    test('returns false', () => {
      expect(result).toStrictEqual(false)
    })
  ...
  describe('event is not a PR', () => {
    ...
    test('writes to stdout with an error message', () => {
      expect(mockStdOut).toHaveBeenCalledTimes(1)
      expect(mockStdOut).toHaveBeenCalledWith('::error::Not a Pull Request event\n')
    })
    test('returns false', () => {
      expect(result).toStrictEqual(false)
    })
